...
def creat_bill_qiwi(chat_id, callback_id, message_id, sum, name_good, amount):
	...
	#el
	if dop.payments_checkvkl() is None: bot.answer_callback_query(callback_query_id=callback_id, show_alert=True, text='Принять деньги на киви кошелёк в данный момент невозможно!')
	else:
		phone, token = dop.get_qiwidata()
		api = SimpleQIWI.QApi(token=token, phone=phone)
		comm = f'bill|{dop.generator_pw(10)}|'
		with open(f'data/Temp/{str(chat_id)}.txt', 'w', encoding='utf-8') as f:
			f.write(str(phone) + '\n')
			f.write(token + '\n')
			f.write(str(amount)+ '\n')
			f.write(str(sum)+ '\n')
		...
		rurl = f'https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={str(phone)}&amountInteger={str(sum)}&amountFraction=0&extra%5B%27comment%27%5D={comm}&currency=643&blocked[0]=account&blocked[1]=sum&blocked[2]=comment'
...
def check_oplata_qiwi(chat_id, username, callback_id, first_name):
	if chat_id not in he_client:
		return
	with open(f'data/Temp/{str(chat_id)}good_name.txt', encoding='utf-8') as f: name_good = f.read()
	phone = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 0)
	token = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 1)
	amount = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 2)
	price = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 3)
	comm = dop.read_my_line(f'data/Temp/{str(chat_id)}.txt', 4)
...
def creat_bill_btc(chat_id, callback_id, message_id, sum, name_good, amount):
	#if dop.amount_of_goods(name_good) <= int(amount): bot.answer_callback_query(callback_query_id=callback_id, show_alert=True, text='Выберите меньшее число товаров к покупке')
	#el
	if dop.get_coinbasedata() is None: bot.answer_callback_query(callback_query_id=callback_id, show_alert=True, text='Принять деньги на btc кошелёк в данный момент невозможно!')
	else:
...
def check_oplata_btc(chat_id, username, callback_id, first_name):
	if chat_id not in he_client:
		return
	with open(f'data/Temp/{str(chat_id)}good_name.txt', encoding='utf-8') as f: name_good = f.read()
	amount = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 0)
	sum = dop.normal_read_line(f'data/Temp/{str(chat_id)}.txt', 1)
	address = dop.read_my_line(f'data/Temp/{str(chat_id)}.txt', 2)
